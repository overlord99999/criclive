...
async def send_main_menu(target, context, user_id, lang, edit=False):
    ...
    # Always send a fresh message with the main menu keyboard
    try:
        ...
        # Photo failed — send plain text fallback
        try:
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "verify_join":
        ...
        if ok:
            ...
            text_msg = t(lang, "not_joined") + f"\n\n👉 Still need to join: <b>{ch_name}</b>"
            try:
                await query.edit_message_caption(
                    caption=text_msg, parse_mode="HTML",
                    reply_markup=welcome_keyboard(lang)
                )
            except Exception:
                try:
