...
def match_list_keyboard(matches, category, lang="en"):
    ...
    for m in matches:
        ...
        # Keep title short enough for a button
        title = m["title"][:38]
        buttons.append([InlineKeyboardButton(
            f"{emoji} {title}", callback_data=f"match_{m['api_id']}_{category}"
        )])
...
def match_detail_keyboard(api_id, category, lang="en"):
    lang_buttons = [
        ...
        [
            InlineKeyboardButton("🔔 " + ("रिमाइंडर" if lang=="hi" else "Reminder"),
                                 callback_data=f"remind_{api_id}"),
            InlineKeyboardButton("🔄 " + ("रिफ्रेश" if lang=="hi" else "Refresh"),
...
def admin_main_keyboard():
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("📋 Live & Upcoming Matches", callback_data="admin_listmatches"),
        ],
        [
...
def admin_match_links_keyboard(matches):
    """Show matches so admin can pick which one to add a link to."""
    buttons = []
    for m in matches:
